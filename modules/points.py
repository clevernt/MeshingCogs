...
class Points(commands.Cog):
    ...
    async def get_points(
        ...
        if user:
            ...
            await ctx.send(f"/me @{ctx.author.name} -> You have {points} points.")

    @commands.command(name="addpoints")
    async def add_points(
        self, ctx: commands.Context, user: twitchio.PartialChatter, amount: int
    ) -> None:
        user_id = int((await user.user()).id)
        self.database.update_points(
            user_id=user_id, username=user.name, amount=amount, operation="add"
        )
        await ctx.send(
            f"/m @{ctx.author.name} -> Added {amount} point(s) to {user.name}"
        )
    ...
    async def remove_points(
        ...
        user_id = int((await user.user()).id)
        self.database.update_points(
            user_id=user_id, username=user.name, amount=amount, operation="remove"
        )
        await ctx.send(
            f"/m @{ctx.author.name} -> Removed {amount} point(s) from {user.name}"
        )
    ...
    async def set_points(
        ...
        user_id = int((await user.user()).id)
        self.database.update_points(
            user_id=user_id, username=user.name, amount=amount, operation="set"
        )
        await ctx.send(f"/m @{ctx.author.name} -> Set {user.name}'s points to {amount}")
